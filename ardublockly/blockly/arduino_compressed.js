...
Blockly.Arduino.init=function(a){
	...
	Blockly.Arduino.variablestypes_=Object.create(null);
	...
	//console.log(b);
	for(var c in b){
		//Indexa todas las variables y les asocia un tipo definido

		if(Blockly.Arduino.getArduinoType_(b[c]) != 'undef'){
...
Blockly.Arduino.addVariable=function(a,b,c){
	...
	var m = Blockly.Arduino.getArduinoAddType_(Blockly.Arduino.variables_[a]);
	Blockly.Arduino.variablestypes_[a]=m;
	return d;
};
...
Blockly.Arduino.getArduinoAddType_=function(a){
	var b = a.split(" ");
	a = b[0];
	var arrayTypes = ['char', 'int','uint8_t', 'long', 'float', 'bool', 'boolean', 'String'  ];
	var arrayExceptions = ['MPU6050', 'MPU9250','TCS3200', 'SoftwareSerial', 'Servo', 'Madgwick','ArduParser', 'TFTCanvas', 'MotorShield', 'Adafruit_PWMServoDriver'];

	if(arrayTypes.indexOf(a) != -1 ){
		return arrayTypes[arrayTypes.indexOf(a)];
	}else if(arrayExceptions.indexOf(a) != -1  ){
		return arrayExceptions[arrayExceptions.indexOf(a)];
	}else{
		return "";	
	}
}
...
Blockly.Arduino.getArduinoType_=function(a){
	switch(a.typeId){
		case Blockly.Types.SHORT_NUMBER.typeId:return"uint8_t";
		...
		case Blockly.Types.SERVO.typeId:{
			Blockly.Arduino.addInclude('servo', '#include <Servo.h>');
			return"Servo";
		}
		case Blockly.Types.TCS3200.typeId:{
			Blockly.Arduino.addInclude('cs', '#include <TCS3200.h>');
			return"TCS3200";
		}
		case Blockly.Types.MPU6050.typeId:{
			//Blockly.Arduino.addInclude('wire', '#include <Wire.h>');
			Blockly.Arduino.addSetup('Wire_begin', 'Wire.begin();', true);
			Blockly.Arduino.addInclude('mpu6050', '#include <IMU_MPU6050.h>');
			return"MPU6050";
		}
		case Blockly.Types.MPU9250.typeId:{
			//Blockly.Arduino.addInclude('wire', '#include <Wire.h>');
			Blockly.Arduino.addSetup('Wire_begin', 'Wire.begin();', true);
			Blockly.Arduino.addInclude('mpu9250', '#include <IMU_MPU9250.h>');
			return"MPU9250";
		}
		case Blockly.Types.MADGWICK.typeId:{
			Blockly.Arduino.addInclude('madgwick', '#include <MadgwickAHRS.h>\n');
			return"Madgwick";
		}
		case Blockly.Types.PARSER.typeId:{
			Blockly.Arduino.addInclude('ss', '#include <SoftwareSerial.h>');
			Blockly.Arduino.addInclude('parser', '#include <ArduParser.h>');
			return"ArduParser";
		}
		case Blockly.Types.TFT.typeId:{
			Blockly.Arduino.addInclude('spi', '#include <ArduParser.h>');
			Blockly.Arduino.addInclude('adafruit_gfx', '#include <Adafruit_GFX.h>');
			Blockly.Arduino.addInclude('tftcanvas', '#include <TFTCanvas.h>');
			return"TFTCanvas";
		}
		case Blockly.Types.MOTOR.typeId:{
			Blockly.Arduino.addInclude('motorshield', '#include <Adafruit_MotorShield.h>');
			return"MotorShield";
		}
		case Blockly.Types.SERVODRIVER.typeId:{
			Blockly.Arduino.addInclude('wire', '#include <Wire.h>');
			Blockly.Arduino.addInclude('servo_driver', '#include <Adafruit_PWMServoDriver.h>');
			return"Adafruit_PWMServoDriver";
		}
		case Blockly.Types.NEXTION.typeId:{
			return"NeoNextion";
		}
		case Blockly.Types.BLOOM.typeId:{
...
Blockly.Arduino.controls_repeat_ext=function(a){var b=Blockly.Arduino.valueToCode(a,"TIMES",Blockly.Arduino.ORDER_ADDITIVE)||"0",c=Blockly.Arduino.statementToCode(a,"DO"),c=Blockly.Arduino.addLoopTrap(c,a.id),d="",e=Blockly.Arduino.variableDB_.getDistinctName("count",Blockly.Variables.NAME_TYPE);a=b;b.match(/^\w+$/)||Blockly.isNumber(b)||(a=Blockly.Arduino.variableDB_.getDistinctName("repeat_end",Blockly.Variables.NAME_TYPE),d+="int "+a+" = "+b+";\n");return d+("for (int "+e+" = 0; "+e+" < "+a+"; "+
...
Blockly.Arduino['procedures_defreturn'] = function(block) {
  ...
  if (block.getReturnType) {
    ...
    type = Blockly.Arduino.variablestypes_[returnType[1]];
  ...
  //console.log(type);
  if (type != null) {
    returnType = Blockly.Arduino.variablestypes_[returnType[1]];
  }else{
  	returnType = 'void';
  }
...
Blockly.Arduino.procedures_callreturn=function(a){
	for(var b=Blockly.Arduino.variableDB_.getName(a.getFieldValue("NAME"),Blockly.Procedures.NAME_TYPE),c=[],d=0;d<a.arguments_.length;d++)c[d]=Blockly.Arduino.valueToCode(a,"ARG"+d,Blockly.Arduino.ORDER_NONE)||0;return[b+"("+c.join(", ")+")",Blockly.Arduino.ORDER_UNARY_POSTFIX]};
...
Blockly.Arduino.stepper_config=function(a){var b=Blockly.Arduino.PinTypes.STEPPER,c=a.getFieldValue("STEPPER_NAME"),d=a.getFieldValue("STEPPER_NUMBER_OF_PINS"),e=Blockly.Arduino.valueToCode(a,"STEPPER_STEPS",Blockly.Arduino.ORDER_ATOMIC)||"360",f=Blockly.Arduino.valueToCode(a,"STEPPER_SPEED",Blockly.Arduino.ORDER_ATOMIC)||"90",g=[a.getFieldValue("STEPPER_PIN1"),a.getFieldValue("STEPPER_PIN2")];"FOUR"===d&&(g.push(a.getFieldValue("STEPPER_PIN3")),g.push(a.getFieldValue("STEPPER_PIN4")));for(var d=
"int "+c+"["+g.length+"] = {",e="Stepper stepper_"+c+"("+e+", ",h=0;h<g.length;h++)Blockly.Arduino.reservePin(a,g[h],b,"Stepper"),d+=g[h]+", ",e+=g[h]+", ";d=d.slice(0,-2)+"};";e=e.slice(0,-2)+");";Blockly.Arduino.addVariable(c,d,!0);c="stepper_"+c;Blockly.Arduino.addInclude("stepper","#include <Stepper.h>");Blockly.Arduino.addDeclaration(c,e);Blockly.Arduino.addSetup(c,c+".setSpeed("+f+");",!0);return""};
...
Blockly.Arduino.variables_set_type=function(a){
	...
	var c = Blockly.Arduino.getArduinoType_(Blockly.Types[a.getFieldValue("VARIABLE_SETTYPE_TYPE")]);
	var arrayExceptions = ['MPU6050', 'MPU9250','TCS3200', 'SoftwareSerial', 'Servo', 'Madgwick','ArduParser', 'TFTCanvas', 'MotorShield', 'Adafruit_PWMServoDriver'];

	if(arrayExceptions.indexOf(c) > 0 ){
		return["",Blockly.Arduino.ORDER_ATOMIC];
	}else{
		return["("+c+")("+b+")",Blockly.Arduino.ORDER_ATOMIC];	
	}
};